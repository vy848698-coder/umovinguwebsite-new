...
export function usePropertyActions(propertyId?: string) {
  ...
  async function toggleWishlist(id: string): Promise<{ wishlisted: boolean } | 'unauthenticated' | 'error'> {
    const token = typeof window !== 'undefined' ? localStorage.getItem('token') : null;
  ...
  async function toggleSave(id: string): Promise<{ saved: boolean } | 'unauthenticated' | 'error'> {
    const token = typeof window !== 'undefined' ? localStorage.getItem('token') : null;
    if (!token) return 'unauthenticated';
    actionLoading.value = true;
    try {
