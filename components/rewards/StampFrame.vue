  ...
  >
    <div class="sf-ring-outer">
      <div class="sf-ring-inner">
        <!-- Always the UMU house mark, not a per-achievement icon — the
             stamp's identity is the brand seal, the same on every
             achievement; what changes is only the title text below it. -->
        <svg class="sf-icon" viewBox="0 0 43 33" fill="none">
...
// app's teal palette with the UMU house mark + achievement title, used as
// the impression left on the Passport page for every achievement. Not a
// raster image: crisp at any size, recolourable, and every achievement
// renders through the exact same path — only the title text varies.
withDefaults(
  defineProps<{
    title?: string | null
...
.sf-icon {
  width: calc(var(--sf-size) * 0.3);
  height: calc(var(--sf-size) * 0.3 * 33 / 43);
  color: currentColor;
}
.sf-title {
  font-size: calc(var(--sf-size) * 0.098);
  font-weight: 800;
